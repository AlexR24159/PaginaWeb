...
const ModalWrapper = ({
  ...
  const lastInitialFocusRef = useRef(null);
  const titleId = useId();
  ...
  useEffect(() => {
    if (!isOpen) {
      ...
      lastInitialFocusRef.current = null;
      return undefined;
    }

    if (lastInitialFocusRef.current !== initialFocusRef) {
      userMovedFocusRef.current = false;
      lastInitialFocusRef.current = initialFocusRef;
    }
    ...
    const handleFocusIn = (event) => {
      ...
      if (initialFocusRef?.current && target === initialFocusRef.current) {
        return;
      }
  ...
  useEffect(() => {
    ...
    scheduleFocus(() => {
      ...
      if (
        userMovedFocusRef.current &&
        activeElement instanceof HTMLElement &&
        modalNode.contains(activeElement) &&
